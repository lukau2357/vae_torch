...
$$X = \{X_{1}, \ldots X_{b}\} \text{ samples taken from the dataset}\\(\mu_{i}, \log(\sigma_{i})) = Encoder_{\phi}(X_{i})$$

Afterwards we compute the weighted average of obtained vectors:
$$\mu' = \sum_{i=1}^{b}\mu_{i}w_{i} \\\log(\sigma') = \sum_{i=1}^{b}\log(\sigma_{i})w_{i}$$
